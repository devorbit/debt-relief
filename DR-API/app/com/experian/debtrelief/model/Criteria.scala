...
case class Criteria(pin: Long = 0, reasonCd: String="", filePath: String ="", needDetails: String ="")
...
object Criteria {
  ...
  implicit val readCrietria: Reads[Criteria] = (
    (JsPath \ "pin").read[Long].filter(JsonValidationError("Pin should not be empty."))(_ > 0) and
      ...
      (JsPath \ "filePath").read[String].filter(JsonValidationError("File Path should not be empty."))(_.length > 0) and
